...
class TNPortOut
    ...
    @ent_tns_removed = Array.new
      ...
      @group_tns_removed = Array.new
			group_tns_hash.each do |tn,tn_attrs|
				...
				yield @tn_action[level][:oci_cmd],{tn: tn, ent: ent, group: group, tn_attrs: tn_attrs, slot: slot}
      end
  ...
  def is_group_clid(config_hash)
    puts "#{config_hash[:ent]},#{config_hash[:group]},#{config_hash[:tn]}, is group CLID, skipping"
  end
    ...
    cleanup_remove(config_hash[:tn],cmd_ok)
	end
  ...
  def cleanup_remove(tn,cmd_ok)
    puts "REMOVED? #{cmd_ok}"
    @group_tns_removed.push(tn) if cmd_ok == true
  end

	def collect_group_tns(config_hash)
  ...
  def remove_from_group(ent,group)
    ...
    @ent_tns_removed += @group_tns_removed if cmd_ok == true
    puts "#{ent},#{group},Removed a total of #{@group_tns_removed.length} GROUP TNs successfully? #{cmd_ok}"
  end

  def collect_from_ent(config_hash)
    puts "removing #{config_hash[:tn]} from ent: #{config_hash[:ent]}"
  end

	def remove_from_ent(ent)
    puts "#{__method__}, ENT TN LIST:\n#{@ent_tns_removed}"
    puts "#{ent},Would have removed #{@ent_tns_removed.length} TN's from #{ent}"
	end
end
